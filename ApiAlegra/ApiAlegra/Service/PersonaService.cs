...
namespace ApiAlegra.Service
{
    public class PersonaService
    {
        ...
        public async Task<(bool Success, string? Content, string? ErrorMessage)>
            CrearPersona(string usuario, string token, CrearClienteRequest personaRequest)
        {
            ...
            var url = "https://api.alegra.com/api/v1/contacts";
            var jsonBody = JsonSerializer.Serialize(personaRequest);
        ...
        public async Task<(bool Success, string? Content, string? ErrorMessage)>
            CrearEmpresa(string usuario, string token, CrearEmpresaRequest EmpresaRequest)
        {
            var authString = $"{usuario}:{token}";
            var authBase64 = Convert.ToBase64String(Encoding.UTF8.GetBytes(authString));
            ...
            var url = "https://api.alegra.com/api/v1/contacts";
            var jsonBody = JsonSerializer.Serialize(EmpresaRequest);
            var content = new StringContent(jsonBody, Encoding.UTF8, "application/json");
        ...
        public async Task<(bool Success, string? Content, string? ErrorMessage)>
            ConsultarPersonaDocumento(string usuario, string token, string identificacion)
        {
            var authString = $"{usuario}:{token}";
            var authBase64 = Convert.ToBase64String(Encoding.UTF8.GetBytes(authString));
            var url = $"https://api.alegra.com/api/v1/contacts?order_direction=ASC&identification={identificacion}";
        ...
        public async Task<(bool Success, string? Content, string? ErrorMessage)>
            ConsultarPersona(string usuario, string token)
        {
            var authString = $"{usuario}:{token}";
            var authBase64 = Convert.ToBase64String(Encoding.UTF8.GetBytes(authString));
            var url = $"https://api.alegra.com/api/v1/contacts?order_direction=ASC";
            string content = string.Empty;
            _httpClient.DefaultRequestHeaders.Clear();
            _httpClient.DefaultRequestHeaders.Add("accept", "application/json");
            _httpClient.DefaultRequestHeaders.Add("Authorization", $"Basic {authBase64}");
